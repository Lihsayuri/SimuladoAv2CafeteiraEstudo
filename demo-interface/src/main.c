...
volatile int estabelece_comunicacao = 0;
volatile int falhou = 0;
...
static void task_receive(void *pvParameters) {
  ...
  for (;;)  {
    ...
    if (p_do == 1) {
	  ...
	  if (p_data[2] == 255){
		  estabelece_comunicacao = 1;
		  xQueueSend(xQueueData, &estabelece_comunicacao, 10);
	  } else if (p_data[2] == 0){
		  falhou = 2;
		  xQueueSend(xQueueData, &falhou, 10);
	  }
...
static void task_main(void *pvParameters) {
  ...
  char p[] = {'U', 0, 0, 'X'};
  char p_cobrar[] = {'U', 1, 2, 'X'};
  char p_verifica[] = {'U', 2, 0, 'X'};
  int data;

  enum states {EXIBE_TELA1, INICIAL, EXIBE_TELA2, CONFIGURANDO, EXIBE_TELA3, PAGAMENTO, EXIBE_TELA4, PREPARANDO, CANCELADO, AGUARDANDO, CONFIGESPERA, VERIFICAPAG, CANCELA} state;
  for (;;)  {
    switch (state)
    {
      case EXIBE_TELA1:
	  ...
	  case AGUARDANDO:
	  if (xQueueReceive(xQueueData, &data , 200)){
		  if (data == 1){
			  state = EXIBE_TELA2;
		  } else {
			  send_package(p, 4);
	  ...
	  case VERIFICAPAG:
	   if (xQueueReceive(xQueueData, &data , 10000)){
		   if (data == 1){
			   state = EXIBE_TELA4;
		   } else if (data == 2){
			   state = CANCELA;
...
void lv_page_1_inicial(void)
{
  ...
  lv_obj_set_style_local_text_color(labelDebug, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_BLACK);
...
void lv_page_cancela(void)
{
	...
	lv_obj_set_style_local_text_font(label_preparando, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, &lv_font_montserrat_20);
	lv_obj_set_style_local_text_color(label_preparando, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_BLACK);
	lv_label_set_text_fmt(label_preparando, "CANCELA TUDO");
...
int main(void) {
  ...
  xQueueData = xQueueCreate(100, sizeof(int));
  if (xQueueData == NULL){
    printf("falha em criar a queue xQueueData \n");
  }
